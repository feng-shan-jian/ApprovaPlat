...
ApprovaPlat 离成熟的审批中台还有不少工作。下面列的是仓库里已经实现的能力，不是提前写好的路线图。
...
下面这些事情目前还不能被写成“已经完整支持”：

- 首个正式数据库版本只支持空 schema 全新安装，不承诺从未发布的开发期数据库自动升级。
- `flowable.database-schema-update=false` 是固定边界，生产结构必须通过正式 SQL 和只读验收管理。
- `ComplexGateway` 暂不支持部署；原生标准循环只允许 XML 往返，业务重复审批使用项目提供的受控循环能力。
- 异步执行器默认关闭。启用定时器、SLA 和后台任务前，需要完成数据库、拓扑、容量、监控和唯一执行协调验收。
- 多节点部署、共享附件存储、真实外部副作用、备份恢复和长时间稳定性，需要在实际目标环境中继续验证。

更细的行为边界见[审批业务契约](docs/contracts/workflow-behavior.md)和[多池协作契约](docs/contracts/workflow-collaboration.md)。
