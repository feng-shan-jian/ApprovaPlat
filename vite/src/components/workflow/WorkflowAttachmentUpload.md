...
`WorkflowAttachmentUpload` 是流程表单的私有附件控件。它通过 `/workflow/attachment/**` 完成临时上传、授权下载和未绑定附件删除，不生成可公开访问的静态文件地址。移除已绑定附件时只解除当前表单字段引用，正式审计文件仍保留在服务端。
...
- 附件状态既不是 `TEMP` 也不是 `BOUND` 时拒绝移除，避免对异常生命周期数据执行错误操作。
