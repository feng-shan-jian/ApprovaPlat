...
readonly GATE_REQUIRED_RELEASE_SQL='flowable/business/8.0.0.3__workflow_attachment_cleanup_retry.sql'
...
#
# 校验增量 SQL 执行顺序，固定本版本唯一允许执行的附件清理重试迁移。
# 参数：$1（字符串）发布目录；$2（字符串）目录业务名称。
# 返回：0 表示顺序文件非空、仅含固定迁移，且该迁移已进入哈希清单。
#
gate_validate_release_order() {
  ...
  [[ "$line_number" -eq 1 \
     && -n "${ordered_sql[$GATE_REQUIRED_RELEASE_SQL]+present}" ]] \
    || gate_die "$label release-order.txt must contain only the approved 8.0.0.3 migration"
}
...
gate_validate_release_configuration() {
  ...
  gate_require_env_assignment "$environment_file" 'RUOYI_BOOTSTRAP_ADMIN_ENABLED' 'false' \
    "$label bootstrap admin example state"
  gate_require_env_assignment "$environment_file" 'RUOYI_BOOTSTRAP_ADMIN_PASSWORD' '' \
    "$label bootstrap admin password example"
...
# 参数：$1（字符串）规范化根目录；$2（字符串）目录业务名称。
# 返回：0 表示未发现 token、bootstrap、Druid、Basic/Bearer、Cookie 或环境凭据泄漏。
#
gate_validate_sensitive_content() {
  ...
  sensitive_assignment_pattern='(RUOYI_TOKEN_SECRET|RUOYI_BOOTSTRAP_ADMIN_PASSWORD|RUOYI_DB_PASSWORD|SPRING_DATA_REDIS_PASSWORD|DRUID_MONITOR_PASSWORD)[[:space:]]*=[[:space:]]*[^[:space:]#$]|(token[._-]?secret|bootstrap[_a-z.-]*password|druid[_a-z.-]*password)[[:space:]]*[:=][[:space:]]*[^[:space:]#$]'
...
#
# 校验全新安装备份已真实生成并在 69 张恢复表上完成 mysqlcheck。
# 参数：$1（字符串）全新安装证据目录；$2（字符串）证据名称。
# 返回：0 表示备份摘要格式正确，且 mysqlcheck 恰好记录 69 个 OK 结果。
#
gate_validate_backup_restore() {
  ...
  awk '
    ...
    NF > 0 { rows++ }
    END { exit(!invalid && rows == 69 ? 0 : 1) }
  ' "$mysqlcheck_file" \
    || gate_die "$label mysqlcheck must contain exactly 69 OK table results"
}
...
gate_validate_evidence_profile() {
  ...
  if [[ "$profile" == 'fresh-install' ]]; then
    ...
    gate_require_exact_line "$evidence_dir/table-counts.tsv" \
      $'69\t20\t11\t32\t6' 'table-counts.tsv'
