...
#
# 校验全新安装备份已真实生成并在 101 张恢复表上完成 mysqlcheck。
# 参数：$1（字符串）全新安装证据目录；$2（字符串）证据名称。
# 返回：0 表示备份摘要格式正确，且 mysqlcheck 恰好记录 101 个 OK 结果。
#
gate_validate_backup_restore() {
  ...
  awk '
    ...
    NF > 0 { rows++ }
    END { exit(!invalid && rows == 101 ? 0 : 1) }
  ' "$mysqlcheck_file" \
    || gate_die "$label mysqlcheck must contain exactly 101 OK table results"
}
...
# 返回：0 表示 57 个正式检查恰好各出现一次且全部通过；异常时终止门禁。
#
  ...
  awk -F '\t' '
    function detail_is_valid(check_name, detail) {
      ...
      if (check_name == "workflow_schema_table_counts")
        return detail == "total=101, ruoyi=20, quartz=11, flowable=36, workflow=34"
      if (check_name == "workflow_retired_table_absence")
        return detail == "unexpected=none"
      if (check_name == "workflow_deployment_artifact_integrity")
      ...
      if (check_name == "workflow_participant_rule_tables")
        return detail == "present=1, missing=none"
      ...
      if (check_name == "workflow_bpmn_event_constraints")
        return detail == "found=13, expected=13"
      if (check_name == "workflow_bpmn_event_data_integrity")
        return detail == "issues=0"
      if (check_name == "workflow_menu_count")
        return detail == "rows=99, natural_keys=99"
      if (check_name == "workflow_menu_tree")
        return detail == "directories=3, extension_parent=1, pages=21, buttons=75, common_management_pages=5, extended_management_pages=9, office_pages=7, invalid_routes=0"
      if (check_name == "workflow_extended_management_role_visibility")
        return detail == "missing_parent_assignments=0"
      if (check_name == "workflow_retired_permissions")
        return detail == "legacy_rows=0"
      if (check_name == "workflow_roles")
        return detail == "active_roles=5, duplicate_roles=0, assignments=workflow_admin:99,workflow_approver:18,workflow_auditor:25,workflow_designer:48,workflow_starter:21"
      if (check_name == "workflow_admin_menu_scope")
        return detail == "assigned=99, expected=99"
    ...
    BEGIN {
      ...
      required["2|workflow_retired_table_absence"] = 1
      required["2|workflow_deployment_artifact_integrity"] = 1
      ...
      required["3|workflow_extended_management_role_visibility"] = 1
...
gate_validate_evidence_profile() {
  ...
  if [[ "$profile" == 'fresh-install' ]]; then
    ...
    gate_require_exact_line "$evidence_dir/table-counts.tsv" \
      $'101\t20\t11\t36\t34' 'table-counts.tsv'
