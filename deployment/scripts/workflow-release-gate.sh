...
#
# 校验全新安装备份已真实生成并在 90 张恢复表上完成 mysqlcheck。
# 参数：$1（字符串）全新安装证据目录；$2（字符串）证据名称。
# 返回：0 表示备份摘要格式正确，且 mysqlcheck 恰好记录 90 个 OK 结果。
#
gate_validate_backup_restore() {
  ...
  awk '
    ...
    NF > 0 { rows++ }
    END { exit(!invalid && rows == 90 ? 0 : 1) }
  ' "$mysqlcheck_file" \
    || gate_die "$label mysqlcheck must contain exactly 90 OK table results"
}
  ...
  awk -F '\t' '
    function detail_is_valid(check_name, detail) {
      ...
      if (check_name == "workflow_schema_table_counts")
         return detail == "total=90, ruoyi=20, quartz=11, flowable=36, workflow=23"
      ...
      if (check_name == "workflow_menu_count")
         return detail == "rows=76, natural_keys=76"
      if (check_name == "workflow_menu_tree")
         return detail == "directories=2, pages=18, buttons=56, invalid_routes=0"
      ...
      if (check_name == "workflow_admin_menu_scope")
         return detail == "assigned=76, expected=76"
...
gate_validate_evidence_profile() {
  ...
  if [[ "$profile" == 'fresh-install' ]]; then
    ...
    gate_require_exact_line "$evidence_dir/table-counts.tsv" \
      $'90\t20\t11\t36\t23' 'table-counts.tsv'
