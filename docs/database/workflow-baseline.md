...
三组脚本共定义 46 项只读检查，所有结果都必须为 `PASS`。表数检查固定核对总表数 95、分项表数 `20/11/36/28`，受控循环、BPMN 错误/升级和审批 SLA 正式表均归入 ApprovaPlat `wf_*`；还必须核对菜单 82 条以及应用账号只拥有目标 schema 的最小 DML 权限。
