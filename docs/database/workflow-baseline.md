...
- `sys_mail_config` 只允许 `config_id=1`。安装和迁移创建空表，首次业务保存写入 revision 1，后续以 revision 条件更新。
- SMTP 授权码以 AES-256-GCM 密文保存；加密子密钥从 RuoYi Token 密钥按固定用途派生，每次加密使用独立 12 字节随机 IV。数据库和查询接口返回公开配置、密文状态和 revision。
- `wf_task_sla_execution.status=COMPLETED` 表示 SLA 时钟已经关闭，审批业务结果由流程状态独立表达；受控整组退回和重提在对应 `wf_task_sla_audit` 保存固定撤销详情，以区别正常任务完成。
- `wf_task_sla_audit.sla_execution_id` 使用 `ON DELETE CASCADE`，SLA execution 满足保留条件后由数据库同步删除关联审计。
