...
public class TokenService
{
    ...
    /**
     * 在应用对外提供登录能力前校验 HS512 密钥长度，阻止弱密钥或空值进入运行态。
     * @return void，无返回值；密钥不足 64 字节时中止应用启动
     */
    @PostConstruct
    public void validateSecret()
    {
        int secretBytes = secret == null ? 0
                : secret.getBytes(StandardCharsets.UTF_8).length;
        if (secretBytes < MIN_TOKEN_SECRET_BYTES)
        {
            throw new IllegalStateException("RUOYI_TOKEN_SECRET 必须至少包含 64 个 UTF-8 字节");
        }
    }
