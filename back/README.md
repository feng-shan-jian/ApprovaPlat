...
Flowable 8 的正式 SQL 来源、执行顺序和未来迁移规则见 `../docs/database/workflow-baseline.md`。首个正式版本只支持全新数据库安装：依次建立 Common、Process、History、DMN、模型版本并发门禁、44 张 `wf_*` 业务表和 98 条菜单记录，最终形成 111 张正式表。开发期 `.1`～`.14` 迁移不属于发布资产；首个基线发布后的结构变化才新增不可改写的正式增量。应用配置固定关闭 schema 自动更新。
