...
`WorkflowProcessInstanceQueryService` 负责“我发起的流程”、管理员实例列表、抄送列表和抄送首次阅读状态。它统一装载历史实例、实时挂起状态、当前任务、正式部署分类和历史发起人名称。可发起定义与用户任务工作台分别由对应查询服务承担。

普通实例和抄送查询始终由服务端固定当前用户。管理员列表把经过校验的 `startUserId` 作为筛选条件，当前身份仍以服务端认证结果为准。
...
| `WorkflowIdentityResolver` | 解析服务端认证的当前有效用户，并作为查询身份的唯一来源 |
| `WfCopyMapper` | 查询当前接收人的抄送记录并原子写入首次阅读状态 |
| `ISysUserService` | 将历史发起人主键解析为当前显示名称 |

## 关键设计

- 当前页实例使用一次有界 `processInstanceIdIn` 任务查询、一次部署查询和一次运行实例查询，以固定批次完成上下文装载。
- 当前任务查询覆盖活动和挂起实例的真实当前环节；任务按创建时间和任务主键稳定排序。
- 单个实例最多接受 200 个当前任务，实例、部署或运行状态批量结果异常时整页返回稳定错误。
- 抄送 Mapper 返回后再次核验 `user_id`；记录缺失与越权统一返回 404，保持相同对象可见性语义。
...
class MyProcessFacade
{
    ...
    {
        // 服务根据当前认证身份固定发起人范围，filter 仅承载业务筛选条件。
