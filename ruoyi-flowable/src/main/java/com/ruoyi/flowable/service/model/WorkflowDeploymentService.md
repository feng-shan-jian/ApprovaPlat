...
| `deleteDeployments(deploymentIds)` | 仅删除活动申请草稿、运行实例和历史实例引用数均为零的部署 |
...
2. 在部署行锁保护下，对活动申请草稿、运行实例和历史实例分别执行 `LIMIT 1 FOR UPDATE` 当前读，任一引用存在即返回 409；活动草稿作为删除保护继续保留。
3. 锁定当前读直接替换普通 count，使已经建立 `REPEATABLE_READ` 事务快照的身份核验链仍能看见等待部署锁期间提交的草稿、运行实例和快速结束历史。
...
7. 清空模型的最近部署关联。
8. 调用 `RepositoryService.deleteDeployment(id)`，部署删除固定采用非级联语义。

上述步骤位于同一个 Spring 事务。草稿创建同样先锁 `ACT_RE_DEPLOYMENT` 再写 `wf_process_draft`：创建先提交时，删除看到 ACTIVE 草稿并返回冲突；删除先提交时，等待中的创建发现部署锁行消失并返回冲突。活动草稿始终保留；快照行数变化、并发产生实例、引擎删除失败都会使业务表、模型关联和引擎表一起回滚。

## 状态约束

- 请求状态只接受 `active` 和 `suspended`。
- 激活已经激活的定义、挂起已经挂起的定义均返回 409，响应准确反映状态冲突。
- 状态命令同步激活或挂起该定义下的运行实例，定义与运行实例保持一致状态。
