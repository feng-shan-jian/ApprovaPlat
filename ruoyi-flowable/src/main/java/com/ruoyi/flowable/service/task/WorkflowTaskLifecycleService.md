...
取消请求可传入根实例或活动 CallActivity 子实例 ID，但服务始终以根实例为业务授权和终止边界。任务服务只查询请求实例并写活动任务 comment，流程实例服务统一冻结根及全部活动子实例，仅删除根实例以便 Flowable 级联结束子流程；写后一次性核对整棵树不存在运行实例、execution 或 task，根历史双状态必须为 `canceled`。任何父子关系漂移、部分删除或终态不一致都会回滚整个 Spring 事务。
