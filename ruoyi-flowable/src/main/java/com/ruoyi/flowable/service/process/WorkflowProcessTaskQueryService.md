...
`WorkflowProcessTaskQueryService` 负责当前用户的活动待办、可认领任务、真实已办任务和已办导出。它基于 Flowable 任务事实装载定义、历史实例、部署分类及发起人显示名称，不承担任务认领、审批、撤回写操作或流程实例列表查询。

待办和已办身份始终由服务端固定。待签列表还会实时复核当前用户是否同时具备认领、待办、查询和审批所需的完整权限，菜单可见性不能替代业务权限。
...
| `listCompletedForExport(filter, pageNum, pageSize)` | `POST /workflow/process/finishedExport` | 与已办列表相同身份、筛选和分页口径，但跳过 Excel 不需要的撤回能力计算 |
...
- 每个非空任务页分别以一次定义查询、一次历史实例查询和一次部署查询批量装载上下文，转换视图时不再访问这些引擎查询。
- 定义缺失保持 404；历史实例、部署缺失或任务关系不一致保持 500，禁止返回缺少上下文的任务。
- 分类筛选只使用 `Deployment.category` 对应的部署主键，未知分类返回真实空页，不退化为全量查询。
- `listCompleted` 逐项调用正式撤回校验，保留身份、实例状态、历史竞态和 BPMN 拓扑合同；导出入口只省略 `revocable` 计算。
- 用户不存在、昵称为空或历史主键不是数字时回显原始发起人主键，避免篡改历史关联。
