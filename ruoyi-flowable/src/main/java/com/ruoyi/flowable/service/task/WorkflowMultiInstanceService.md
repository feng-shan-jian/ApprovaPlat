...
- 加签和减签只调用 Flowable 8 的 `addMultiInstanceExecution` 与
  `deleteMultiInstanceExecution` 公共 API，`nrOf*` 变量由 Flowable 引擎独占维护。
- revision 变量更新依赖 Flowable 持久化 revision 形成 CAS。加减签固定按“引擎
  revision、业务轮次 CAS、有序成员变量、execution 动作”执行；只有业务 CAS 胜方继续
  写成员变量和 execution。完成链在 `TaskService.complete` 前同步引擎和轮次 revision，
...
- 动态 `ADD`、`REMOVE` 和 `COMPLETE` 的失败方若命中 Flowable 乐观锁、Spring
  ...
  重新查询服务端状态，并使用新 revision 发起一次明确重试。
- 完成命令后继续核对多实例根、活动 task/execution、冻结成员、模式和三项引擎计数；
  正式轮次的完成状态由同步 completion listener 的 Mapper CAS 唯一确认，外层直接使用该结果
  刚更新的轮次。流程直接结束时仍核对已结束历史和来源根消失。
