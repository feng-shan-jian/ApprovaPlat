...
`WorkflowProcessVariableProjection` 是流程详情专用的变量存储与安全投影具体组件。`WorkflowProcessDetailService` 在完成实例/任务对象授权后直接调用它，并继续使用详情入口已经建立的同一只读事务。

组件维护三组必须一起变化的不变量：

- 从 `ACT_HI_DETAIL` 读取正式提交快照时，先完整校验元数据、类型、物理存储关系和累计容量，再分批读取正文并执行固定快照协议解码；
- 从 `ACT_HI_VARINST` 读取活动表单当前值时，先通过 Flowable `excludeVariableInitialization()` 查询限定根变量或当前 task-local 作用域，再用生产 Mapper 核对物理元数据，按需读取 Blob/JSON 正文；
- 所有响应值统一执行内部变量隐藏、JSON 危险键、深度、节点、容器、文本、正文和序列化字节门禁，非法存储或关联矛盾统一失败关闭。

## 接入方式

Spring 通过构造器注入真实 `HistoryService` 和 `WorkflowHistoricVariableMapper`，详情服务直接注入该具体组件。该实现集中承担变量存储协议和安全投影职责。
...
- `loadSubmissionSnapshots(instanceId, deploymentId, ancestorDeploymentIds)`：返回已完成两阶段读取和固定协议解码的 `VariableStore`；正式快照缺席时返回空索引。
...
`ProjectedValues` 同时返回逐字段 JSON 序列化字节数。`WorkflowProcessFormDetailProjection` 把这些字节并入跨历史表单和当前表单共享的整页 `MAX_TOTAL_VARIABLE_BYTES` 预算，职责拆分后继续执行同一资源上限。

## 关键设计约束

- 详情服务完成对象授权后直接调用本组件；HTTP 和公共业务入口统一收敛在详情服务。
- 快照与活动变量正文继续使用生产 `WorkflowHistoricVariableMapper` 的元数据/正文两阶段 SQL，Blob 只按需读取。
- 字符串 Blob 仅允许反序列化单个 `String`；JSON 使用严格重复字段和尾随内容检测。
- 详情响应白名单覆盖安全标量、字符串和受控 JSON；`bytes`、`serializable`、未知类型、自定义对象、内部变量和危险 JSON 键触发数据异常。
- 存储损坏、关联漂移、作用域矛盾或任一资源上限触发时返回原有 HTTP 500 数据异常，并整体终止本次详情投影。
