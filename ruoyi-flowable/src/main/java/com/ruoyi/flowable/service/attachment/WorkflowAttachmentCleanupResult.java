 ...
 * @param leaseLost int，对象处理期间租约过期并被其他节点重领的记录数
 */
public record WorkflowAttachmentCleanupResult(int cleaned, int failures, int leaseLost)
{
    /**
     ...
     * @param leaseLost int，本轮租约所有权丢失数
     * @return 无返回值，参数非法时拒绝创建结果
     */
    public WorkflowAttachmentCleanupResult
    {
        if (cleaned < 0 || failures < 0 || leaseLost < 0)
        {
