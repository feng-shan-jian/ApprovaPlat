...
普通用户的权限可由全部有效角色并集满足，不要求集中在同一角色。`claim` 是 `approval` 的严格超集；仅有直接办理权限的用户不能进入普通多人候选计划，否则任务会成为其无法查看或认领的无人可办任务。

任一用户缺少 `approval` 时整批返回 HTTP 400 `所选用户不存在、已停用或无流程办理权限`；普通多人计划中任一用户缺少 `claim` 时整批返回 HTTP 400 `所选候选用户不存在、已停用或无完整认领权限`。两种失败都发生在来源任务完成和身份写入前，流程变量、后继任务与审计不发生变化。

## 支持范围

- 实例当前只有来源任务一个活动任务。
- 来源节点为主流程非多实例用户任务；动态多实例初始化前驱必须恰有一条入边，禁止并行 token 创建多个来源任务。
...
条件分支、网关、事件、子流程边界、串行/自由表达式多实例、额外活动任务或监听器改变 owner 的场景均拒绝动态指定，并返回 `409`。
