...
`WorkflowTaskMovementPolicy` 负责只读分析已部署的 BPMN 模型，Flowable 数据写入由对应 ApplicationService 执行。它为驳回、退回和撤回提供保守的串行迁移门禁，使状态变更命令限定在安全串行作用域内。
...
- 静态多实例、损坏的 SequenceFlow、循环/回边以及超过遍历预算的图同样失败关闭；路径安全完全由部署拓扑证明。
- 退回目标由服务端从实例真实历史确定为首个审批节点，客户端提交原任务与业务意见。
- 驳回目标由服务端确定为唯一主流程结束节点，当前任务到该节点的路径必须安全。
- 撤回只允许来源用户任务的直接同步普通用户任务后继；并行网关必须形成可证明的纯分支，且每个分支只有一个无条件普通用户任务。分析结果以不可变 `RevokeMovementPlan` 返回。
- BPMN 图遍历有硬上限，异常模型返回状态冲突；迁移只使用完整验证后的确定路径。
...
调用方必须在同一事务内重新查询任务、流程实例、历史和执行树，并在真正执行 `changeState()` 前由服务端确定首审批节点，再使用与来源类型对应的策略校验路径。该类证明 BPMN 拓扑可安全重放；受控多实例成员、计数、轮次和并发事实由轮次服务负责。目标选择完全由服务端事实决定。
