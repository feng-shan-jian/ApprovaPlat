...
用户任务必须至少配置 `assignee`、`candidateUsers` 或 `candidateGroups` 之一；`owner` 本身不能代替真实办理身份。任一静态引用不存在、停用、删除或缺少相应资格时，整次部署拒绝，不返回部分成功。

## 表达式与运行时校验

包含 `${...}` 或 `#{...}` 的身份由 Flowable 在运行时解析，部署时不伪造变量结果。任务 `create` 监听由 `WorkflowUserTaskAuditService` 读取引擎已生成的 candidate identity links，对动态候选用户和每个候选组重新执行完整 `claim` 资格校验；校验失败会回滚任务创建事务。

## 编码和异常语义

- 静态 ID 拒绝空白、前导零、符号、小数、零、负数和 `long` 溢出。候选组前缀区分大小写，不兼容 `ROLE007`、`DEPT003`、`ROLE_7` 或未知前缀。
- 用户任务名称或节点 ID 会进入稳定业务错误，便于设计者定位。
- 文档为空、身份格式非法、主数据无效或资格不足返回 `409`，且不会创建部署、流程定义或任务。数据库/映射层技术异常保留为 `500`。

## 关键设计

校验器先分类收集所有流程和子流程中的用户任务身份，再执行有界批量查询。存在性与资格分开校验，使错误能区分“主数据失效”和“无法走通真实业务入口”。角色和部门按每个组独立核验，不允许同批其他有效组掩盖无人可认领的组。
