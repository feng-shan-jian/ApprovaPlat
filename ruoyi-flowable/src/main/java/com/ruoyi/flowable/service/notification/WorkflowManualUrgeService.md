...
- `urge(WorkflowManualUrgeRequest request)` 返回 `WorkflowManualUrgeView`，包含本次接收人数和通知通道记录数；兼容字段名仍为 `outboxCount`。
...
- 服务不直接写通知 SQL，通过 `WorkflowNotificationService.registerManualUrge` 登记。
