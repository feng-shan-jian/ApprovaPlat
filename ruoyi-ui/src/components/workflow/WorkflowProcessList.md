...
- KeepAlive 首次挂载触发的 `activated` 只登记激活状态，由初始化请求负责首屏数据，避免首次进入重复查询；详情页完成退回、重提或其他任务动作并关闭页面后，缓存中的来源列表才会在后续重新激活时回读服务端。若回页发生在首次分类加载结束前，则在初始化完成后补做一次查询，不会继续展示已经撤销的旧任务或旧流程状态。
