...
import { createParticipantRuleDomain, isParticipantRuleProperty } from './participantRuleDomain.js'
...
export function isFormParticipantProperty(name) {
  return isParticipantRuleProperty(name)
...
export function createFormParticipantDomain(context) {
  ...
  function updateMultiInstance() {
    ...
    const editableWithPlatformProperties = preservedPlatformProperties
      .filter(item => !isParticipantRuleProperty(String(item?.name || '')))
