...
`UserTaskSlaEditor` 为 BPMN `UserTask` 提供结构化审批 SLA 配置。组件只接受正式业务日历、具备审批能力的用户和启用的 BPMN Escalation 编码，不开放定时表达式、任意接收人或任意事件编码。
...
- 升级目标只允许正式用户或受控 Escalation 编码，避免作者注入表达式和任意处理器。
- 启用前必须加载到正式日历和至少一个升级目标；首次启用自动选中首个合法目录项，避免提交半成品作者配置。
