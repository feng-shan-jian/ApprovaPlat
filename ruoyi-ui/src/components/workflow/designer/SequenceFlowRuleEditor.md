...
`SequenceFlowRuleEditor` 是排他网关和包容网关出线的受控规则编辑器。组件接收当前流程正式表单的字段目录，按字段类型提供运算符和值控件，并输出后端 `WorkflowConditionDeploymentService` 可校验的版本化规则 JSON；受控规则 JSON 直接替换任意 EL 输入。
...
- 数值、布尔、枚举和文本字段使用不同运算符和值控件；切换字段时重置字段值并按新类型重新选择。
- 默认分支的可执行条件固定为空，并保存 `{ version: 1, default: true }` 配置；其他分支均必须完整配置。
