...
- 组件只维护当前编辑草稿；后端目录、Modeler 命令和持久化均由父组件负责。
- 从作者 XML 回读的 JSON 解析或结构校验失败时，组件进入显式错误态并保留原 XML。
- 输入变量和结果变量使用同一标识符白名单，Flowable 多实例变量与平台内部前缀归入保留字集合。
- 每次提交会裁剪字段首尾空白、按变量名排序并固定 JSON 字段顺序，便于部署时与服务端规范配置比对。
- 客户端结构校验用于即时反馈；CEL 静态类型、表达式资源限制、版本校验和及运行值类型仍由服务端 `WorkflowCelSandbox` 强制执行。
- 结果变量与输入变量使用互斥命名，确保后续节点持续按原输入契约读取。
