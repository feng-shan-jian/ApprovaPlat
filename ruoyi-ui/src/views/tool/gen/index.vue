...
/**
 * 根据单表行数据或当前批量选择生成代码。
 * @param {Record<string, any> | MouseEvent} row 单表配置；批量按钮触发时为浏览器点击事件。
 * @returns {void} 无返回值，生成结果写入自定义路径或通过真实下载接口返回压缩包。
 */
function handleGenTable(row) {
  // tbNames 表示单个表名或批量勾选后的逗号分隔表名。
  ...
  if (row.genType === "1") {
    ...
    // 批量生成使用平台级文件名，单表生成保留表名以便追踪。
    const zipName = Array.isArray(tbNames) ? "approvaplat.zip" : tbNames + ".zip"
